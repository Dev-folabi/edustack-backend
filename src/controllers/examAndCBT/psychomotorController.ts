 ...
 */
export const savePsychomotorAssessments = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (!staff) {
      return handleError(
        res,
        "Forbidden: User is not a registered staff member.",
        403
      );
    }
    ...
    if (!Array.isArray(assessments) || assessments.length === 0) {
      return handleError(
        res,
        "Request body must be a non-empty array of assessments.",
        400
      );
    }
    ...
    logger.info(
      { count: assessments.length, assessedBy: staffId },
      "Psychomotor assessments saved successfully."
    );
