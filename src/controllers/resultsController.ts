 ...
 */
export const addManualMarks = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { studentId, examPaperId, marksObtained, teacherRemark } = req.body;

        const examPaper = await prisma.examPaper.findUnique({ where: { id: examPaperId } });
        if (!examPaper) {
            return handleError(res, "Exam paper not found.", 404);
        }
        if (examPaper.mode !== 'PaperBased') {
            return handleError(res, "This endpoint is only for paper-based exams.", 400);
        }
        ...
        logger.info({ resultId: result.id, studentId, examPaperId }, "Manual marks added successfully.");
 ...
 */
export const addTermRemarks = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { studentId, termId, sessionId, classTeacherRemark, schoolHeadRemark } = req.body;
        ...
        if (!enrollment) {
            return handleError(res, "Student enrollment for the specified term/session not found.", 404);
        }
        ...
        logger.info({ enrollmentId: updatedEnrollment.id }, "Term remarks added successfully.");
 ...
 */
export const finalizeCbtResults = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!attempts.length) {
            return handleError(res, "No submitted attempts found to finalize for this exam paper.", 404);
        }
        ...
        for (const attempt of attempts) {
            const allEssaysGraded = attempt.responses.every(r => r.marksAwarded !== null);
            if (!allEssaysGraded) {
                return handleError(res, `Cannot finalize results. Attempt ID ${attempt.id} has ungraded essay questions.`, 400);
            }
        ...
        logger.info({ examPaperId, count: resultsToCreate.length }, "CBT results finalized successfully.");
...
export const publishResults = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (typeof publish !== 'boolean') {
            return handleError(res, "The 'publish' field must be a boolean value.", 400);
        }
        ...
        logger.info({ examPaperId, published: publish, count }, "Results publication status changed.");
 ...
 */
export const getEssayResponsesForGrading = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { examPaperId } = req.params;
 ...
 */
export const gradeEssayResponse = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (response.question.type !== 'Essay') {
            return handleError(res, "This response is not for an essay question.", 400);
        }
        if (marksAwarded > response.question.marks) {
            return handleError(res, `Marks awarded (${marksAwarded}) cannot exceed the maximum marks for the question (${response.question.marks}).`, 400);
        }
        ...
        logger.info({ responseId, marksAwarded }, "Essay response graded successfully.");
