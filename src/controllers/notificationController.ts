...
import { NotificationCategory, NotificationType, PrismaClient } from "@prisma/client"; // Import enums for clarity
 ...
 */
const _formatStudentsToRecipients = (students: any[]): Recipient[] => // any[] for flexibility from various includes
  students.map((s) => ({
    id: s.user.id,
 ...
 */
const _formatStaffToRecipients = (staff: any[]): Recipient[] => // any[] for flexibility
  staff.map((s) => ({
 ...
 */
const _getStudentsByIds = async (studentIds: string[]): Promise<Recipient[]> => {
    if (!studentIds || studentIds.length === 0) return [];
    const students = await prisma.student.findMany({
      where: { id: { in: studentIds } },
      include: { user: {select: {id: true, email: true}}, parent: {select: {email: true}} },
    });
 ...
 */
const _getStudentsByClassOrSection = async (classId: string, sectionId?: string): Promise<Recipient[]> => {
    const students = await prisma.student.findMany({
      where: {
        student_enrolled: {
          some: {
...
const _getTargetRecipients = async (filter: {
  ...
  if (filter.classId) {
    recipients.push(...await _getStudentsByClassOrSection(filter.classId, filter.sectionId));
  } else if (filter.schoolId && !(filter.studentIds && filter.studentIds.length > 0)) {
  ...
  } else if (filter.schoolId && !(filter.staffIds && filter.staffIds.length > 0) &&
             !(filter.studentIds && filter.studentIds.length > 0) && !filter.classId) {
  ...
  recipients.forEach(r => {
      if (!uniqueRecipientsMap.has(r.id) || (r.email && !uniqueRecipientsMap.get(r.id)?.email) ) {
...
const _dispatchMessages = async (
    // tx: PrismaTransactionClient, // Prisma transaction client, if operations need to be part of a larger transaction
    ...
    const { title, message, category, channels, scheduledAt, createdById } = details;

    if (scheduledAt) {
        await prisma.scheduled_Message.createMany({ // Not using tx here, as scheduling might be standalone
          data: recipientsToNotify.map((user) => ({
            userId: user.id, title,
            email: user.email || undefined, message, category,
            type: channels.includes(NotificationType.EMAIL) && channels.includes(NotificationType.IN_APP)
                  ? NotificationType.BOTH : channels[0],
            createdById, scheduledAt: new Date(scheduledAt),
          })),
        });
        logger.info({ count: recipientsToNotify.length, createdBy: createdById, scheduledAt }, "Bulk messages scheduled.");
      } else {
        await Promise.all(
          recipientsToNotify.map((user) =>
            notifyUser({
              userId: user.id, email: user.email || undefined,
              title, message, category, channels,
            })
          )
        );
        logger.info({ count: recipientsToNotify.length, createdBy: createdById }, "Bulk messages sent immediately.");
      }
};
 ...
 */
export interface SendBulkMessageRequest { // Renamed for clarity
  recipients: {
    studentIds?: string[]; staffIds?: string[];
    classId?: string; sectionId?: string; schoolId?: string;
  };
...
export const sendBulkMessages = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (usersToNotify.length === 0) {
        logger.warn({recipientsFilter: recipients}, "No recipients found for bulk message criteria.");
    ...
    await _dispatchMessages( usersToNotify, {title, message, category, channels, scheduledAt, createdById});

    res.status(200).json({
      success: true,
      message: scheduledAt ? "Messages scheduled successfully!" : "Messages sent successfully!",
 ...
 */
export const getNotificationsForUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = (req as any).user;
    const { category, status, startDate, endDate } = req.query;

    if (!userId) {
      logger.warn("Attempt to fetch notifications without user ID (unauthenticated).");
    ...
    logger.info({ userId, filterCount: Object.keys(filters).length -1 , resultCount: notifications.length }, "User notifications fetched.");

    res.status(200).json({
      success: true, message: "Notifications fetched successfully.", data: notifications,
    });
  } catch (error) {
    next(error);
  }
